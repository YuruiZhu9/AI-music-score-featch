...
def build_gta_text(
    ...
    lines: List[str] = []
    lines.append("=" * 60)
    lines.append("  AI Guitar Tab — 自动生成的吉他谱")
    lines.append("=" * 60)
    lines.append(f"Tempo: {bpm.get('bpm', 120)} BPM")
    lines.append(f"Time:  {bpm.get('time_signature', '4/4')}")
...
def _build_tab_grid(
    ...
    # 音符时长映射（秒 → 字符数）
    beat_duration = 60.0 / bpm.get("bpm", 120)
